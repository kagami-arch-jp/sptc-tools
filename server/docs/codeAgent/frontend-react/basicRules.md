...
- 对全局状态进行修改的方法，必须写在 `store` 里，**禁止把方法直接写在组件内部**
  **组件内部只允许调用方法和读取状态，不允许直接修改store里的值**
- **禁止父组件引用别的父组件下的子组件**
  - 例如，`src/components/A/index.jsx` 可以引用 `src/components/B/index.jsx`，但不允许引用 `src/components/B/C/index.jsx`，因为组件`A`与`B`平级，所以可以互相引用，而`B/C`属于`B`的子组件，组件`A`不允许引用`B`的子组件
